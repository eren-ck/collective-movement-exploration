...
@upload_page.route('/')
def upload_list():
    datasets = Dataset.query.filter_by(user_id=current_user.id).filter(Dataset.progress != 100)
    return render_template('/center/upload/list.html', datasets=datasets)


@upload_page.route('/new')
def upload_new():
    form = UploadForm()
    # datasets = Dataset.query.filter_by(user_id=current_user.id).filter(Dataset.progress != 100)
    return render_template('/center/upload/new.html', form=form)


# @upload.route('/<user_url_slug>/photos')
# def photos(user_url_slug):
#     # Do some stuff
#     return render_template('profile/photos.html')
#
# @upload.route('/<user_url_slug>/about')
